...
def catgoryItemsJSON(catagory_id):
...
def showCatagories():
    ...
    if 'username' not in login_session:
        return render_template('publicrestaurants.html', catagories=catagories)
    else:
        return render_template('restaurants.html', catagories=catagories)
...
def newCatalog():
    ...
    if request.method == 'POST':
        ...
        flash('New Restaurant %s Successfully Created' % newCatagory.name)
        session.commit()
...
def editCatagory(catagory_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('catagory Successfully Edited %s' % editedcat.name)
            return redirect(url_for('showCatagories'))
    else:
        return render_template('editRestaurant.html', catagory=editedcat)



@app.route('/catagory/<int:catagory_id>/delete/', methods=['GET', 'POST'])
def deleteCatagory(restaurant_id):
    delcat = session.query(
        Catagory).filter_by(id=catagory_id).one()
    if 'username' not in login_session:
        return redirect('/login')
    ...
    if request.method == 'POST':
        ...
        flash('%s Successfully Deleted' % delcat.name)
        session.commit()
        return redirect(url_for('showRestaurants', catagory_id=catagory_id))
    else:
        return render_template('deleteRestaurant.html', catagory=delcat)
...
def showMenu(catagory_id):
    ...
    items = session.query(CatagoryItem).filter_by(
        catagory_id=catagory_id).all()
    if 'username' not in login_session :
        return render_template('publicmenu.html', items=items, catagory=catagory, creator=creator)
    else:
        return render_template('menu.html', items=items, catagory=catagory, creator=creator)

@app.route('/catagory/<int:catagory_id>/menu/new/', methods=['GET', 'POST'])
def newMenuItem(catagory_id):
    if 'username' not in login_session:
        return redirect('/login')
    cat = session.query(Catagory).filter_by(id=catagory_id).one()
    if login_session['user_id'] != cat.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add menu items to this restaurant. Please create your own restaurant in order to add items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        newItem = CatagoryItem(name=request.form['name'], description=request.form['description'], catagory_id=cat_id, user_id=cat.user_id)
        session.add(newItem)
        session.commit()
        flash('New Menu %s Item Successfully Created' % (newItem.name))
        return redirect(url_for('showMenu', catagory_id=cat_id))
    else:
        return render_template('newmenuitem.html', catagory_id=cat_id)


@app.route('/catagory/<int:catagory_id>/menu/<int:menu_id>/edit', methods=['GET', 'POST'])
def editMenuItem(catagory_id, menu_id):
    if 'username' not in login_session:
        return redirect('/login')
    editedItem = session.query(CatagoryItem).filter_by(id=menu_id).one()
    cat = session.query(Catagory).filter_by(id=catagory_id).one()
    if login_session['user_id'] != cat.user_id:
...
def deleteMenuItem(catagory_id, menu_id):
    if 'username' not in login_session:
        return redirect('/login')
    cat = session.query(Catagory).filter_by(id=catagory_id).one()
    itemToDelete = session.query(CatagoryItem).filter_by(id=menu_id).one()
    if login_session['user_id'] != cat.user_id:
